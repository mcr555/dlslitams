...
$sql = "SELECT * FROM hardware supplier_id LEFT JOIN supplier supplier_name ON supplier_id.supplier_id=supplier_name.supplier_id WHERE warranty_expiration >='$gen' AND warranty_expiration <= '$gen1'";
        $result = $conn->query($sql);

 if ($result->num_rows == 0){
		echo "<script>alert('No report found. Please try again'); location.href='../Report1/ReportHardawareWarranty.php';</script>";
		exit(0);
 ...
 session_start();
        ...
        $result1 = $conn->query($sql1);
            ...
            $sql = "INSERT INTO tbl_log(Log_Name, Log_LOP, Log_Date_Time,category, Log_Function,id)VALUES ('$vn $vn1 $vn2','$vn3','$vd','Report','Generate report of hardware waranty date $gen-$gen1 ','')";
...
if ($row['status'] == "5")
	$status = 'Borrowed ';
		...
		$pdf->setX(340);$pdf->Cell(0,0,''.'On warranty',0,0,'L');
